#ifndef UPLINK_ENC_H
#define UPLINK_ENC_H

#include <stdint.h>

#define GEN_POLY  (0xAE3U)
#define MSG_LEN   (144)  // Data size to be encoded (interpreted as payload length) [MUST BE DIVISIBLE BY 3]
#define ENC_LEN   (288)  // Encoder output size in bytes
#define WORD_COUNT (96)  // The amount of Golay code-words: Data/1.5

void encode_data(uint8_t (*in)[MSG_LEN], uint8_t (*out)[ENC_LEN]);

#ifndef SCRAMBLER_ARRAY
#define SCRAMBLER_ARRAY
static const uint8_t scrambler[ENC_LEN] = {
  
};
#endif /* SCRAMBLER_ARRAY */

#endif
