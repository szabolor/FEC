...
void rs_test() {
  ...
  unsigned char RSdecdata[256];

  fp = fopen("enc_rs", "rb");
  fread(rs_data, 1, sizeof(rs_data), fp);
  fclose(fp);
  ...
  for(col=RSPAD;col<NN;col++){
    for(row=0;row<RSBLOCKS;row++){
      //rsblocks[row][col] = rs_data[di++] ^ Scrambler[si++];  /* Remove scrambling */
      rsblocks[row][col] = rs_data[di++];
    }
  }
  /* Run RS-decoder(s) */
  rs_failures = 0;
...
void encode_test() {
  ...
  encode_FEC40(reencode, enc_rs);

  fp = fopen("reenc_conv", "wb");
  fwrite(convolutional_code, 1, sizeof(convolutional_code), fp);
  fclose(fp);

  for (i = 0; i < 650; ++i) {
    reenc_byte[i] = reencode[8*i] << 7 | reencode[8*i+1] << 6 | reencode[8*i+2] << 5 | reencode[8*i+3] << 4 | reencode[8*i+4] << 3 | reencode[8*i+5] << 2 | reencode[8*i+6] << 1 | reencode[8*i+7];
  }

  fp = fopen("reenc_interleaved", "wb");
  fwrite(reenc_byte, 1, sizeof(reenc_byte), fp);
  fclose(fp);
   
}

void fec_generate_test() {
  FILE* fp;
  unsigned char reenc_byte[650];
  unsigned char enc_rs[] = {
    0x53, 0x4d, 0x4f, 0x47, 0x20, 0x74, 0x65, 0x6c, 0x65, 0x6d, 0x65, 0x74,
  ...
  fp = fopen("reenc_fec", "wb");
  fwrite(reencode, 1, sizeof(reencode), fp);
  fclose(fp);
}
